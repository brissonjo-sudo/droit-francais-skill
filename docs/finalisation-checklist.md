...
| Connexion d'une nouvelle installation | un compte qui n'a jamais connecté le plugin y parvient, DCR fermée | client prédéfini déclaré côté hôte ; connexion réussie depuis une installation neuve ; journal Auth0 *Success Exchange* | ◐ **correction du 16/09/2026** : la fermeture de la DCR bloquait toute nouvelle installation, la connexion du 04/09 ne valant que pour le connecteur de test. Claude (claude.ai) ✅ 16/09/2026 par client prédéfini ; Claude Code ☐ non configuré ([#88](https://github.com/brissonjo-sudo/droit-francais-skill/issues/88)) ; **ChatGPT ☐ bloquant avant soumission** ([#89](https://github.com/brissonjo-sudo/droit-francais-skill/issues/89)) — sans client prédéfini ni CIMD, le relecteur OpenAI ne pourra pas se connecter. Voir [`oauth.md`](oauth.md) § 4 |
...
| Maintien hors veille | aucun réveil sur 24 h après mise en service du ping externe | historique du ping sans échec ni trou > 10 min + résumé `--jours 1` sans réveil ; quota d'heures Render vérifié | ◐ **ping en service, daté par la mesure** : mise en service le 03/09/2026 entre 14:24 et 18:32 UTC ; côté journal `surveillance`, 0 réveil sur les 6 sondes suivantes contre 5 réveils sur 5 auparavant, et toujours 0 réveil sur les 67 mesures du 03/09 18:32 au 14/09/2026 13:53 UTC. Reste dû : l'historique du service de ping lui-même et le quota d'heures Render — § 11 des pièces |
| Disponibilité avant publication | couverture sur sept jours sans trou > 6 h ; aucune panne ni dérive ; aucun réveil dépassant 30 s | journal `surveillance`, résumé sur sept jours avec `--exiger-sans-defaut`, qui porte les quatre conditions | ✅ 14/09/2026 — fenêtre du 03/09 18:32 au 10/09/2026 18:32 UTC extraite du journal puis résumée avec `--exiger-sans-defaut` : sortie 0 ; 43 mesures, aucun trou > 6 h, 0 défaut, 0 indisponibilité, 0 réveil ; `/health` à chaud médiane 0,21 s, p95 0,51 s. Hors fenêtre, un trou de 7,8 h le 14/09 (planificateur GitHub, 0 défaut) fait échouer `--jours 7` sur la période suivante : il ne rouvre pas la porte |
...
| Déploiement du correctif de sécurité | `/health` annonce la version qui porte le correctif JWKS | montée en `0.8.1`, fusion, redéploiement Render, puis relecture de `/health` | ✅ 04/09/2026 — le journal `surveillance` relève `0.8.1` dès la mesure de 18:18 UTC (dernière `0.8.0` à 14:14), puis `0.8.2` le 05/09 et `0.8.3` le 14/09/2026 à 13:53 ; relecture directe de `/health` le 14/09 : `0.8.3`, `auth: oauth`, 0,18 s |
...
- **échéance de l'essai Auth0 vers le 21 septembre 2026** — 7 jours restants
  au 14/09/2026. Le locataire n'est pas suspendu, l'offre gratuite s'active
