...
| Sonde Alpine (#34) | santé et métadonnées OAuth conformes ; six outils listés et réellement appelés ; lectures Légifrance/Judilibre avec texte et provenance officielle ; article inexistant non inventé | sonde publique puis `check_live_tools.py` avec jeton M2M éphémère ; Légifrance 1,000 s, Judilibre 0,461 s, version 0.7.0 consignées en E4 | ✅ 02/09/2026 |
| Maintien hors veille | aucun réveil sur 24 h après mise en service du ping externe | historique du ping sans échec ni trou > 10 min + résumé `--jours 1` sans réveil ; quota d'heures Render vérifié | ⏳ voie tranchée le 02/09 (ping externe 5 min) ; mise en place humaine, § 11 des pièces |
| Disponibilité avant publication | couverture sur sept jours sans trou > 6 h ; aucune panne ni dérive ; aucun réveil dépassant 30 s | journal `surveillance`, résumé sur sept jours avec `--exiger-sans-defaut`, qui porte les quatre conditions | ⏳ service sain à chaud (médiane 0,24 s, 0 indisponibilité) ; les sept jours ne courent qu'à partir du ping externe |
...
- création du compte de ping externe et vérification du quota d'heures Render
  (§ 11 des pièces humaines) : le cron GitHub ne tient pas la cadence, un run
  toutes les deux à quatre heures au lieu de douze par heure ;
