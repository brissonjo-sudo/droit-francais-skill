...
| DCR | désactivé si inutile ; sinon politique et limites documentées | activée, nécessaire (ChatGPT s'enregistre lui-même) ; vérifié le 1/9/2026 : grant tiers utilisateur `Authorized` sans permission (`scope: []`, `allow_all_scopes: false`), accès client `Unauthorized`. Le test négatif avec le grant supprimé rend le plugin inaccessible ; après rétablissement et actualisation, six outils visibles et appel Légifrance réel réussi | ✅ |
