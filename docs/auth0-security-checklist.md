...
| DCR | désactivé si inutile ; sinon politique et limites documentées | activée, nécessaire (ChatGPT s'enregistre lui-même) ; observé dans le journal Auth0 le 1/9/2026 à 21:13Z : grant tiers utilisateur `Authorized` sans permission (`scope: []`, `allow_all_scopes: false`), accès client `Unauthorized`. Test négatif avec grant supprimé puis rétablissement, six outils visibles et appel Légifrance réel réussi. Procédure reproductible : [pieces-humaines.md](pieces-humaines.md) § 2 ; capture ou export expurgé à archiver dans le dossier privé | ◐ |
